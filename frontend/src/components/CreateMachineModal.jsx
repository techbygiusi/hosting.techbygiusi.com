...
const TEXT = {
  en: {
    ...
    progress: 'Provisioning progress', technical: 'Live log', done: 'Container is ready.', failed: 'Provisioning failed.', unavailable: 'Container self-service is temporarily unavailable.'
  },
  de: {
    ...
    progress: 'Bereitstellungsfortschritt', technical: 'Live-Protokoll', done: 'Container ist bereit.', failed: 'Bereitstellung fehlgeschlagen.', unavailable: 'Der Container-Self-Service ist vorübergehend nicht verfügbar.'
  }
};
  ...
  if (job) {
    ...
    return <Modal title={t.queued} onClose={onClose}>
      <div className="provisioning-live-view">
        ...
        {job.status === 'failed' && <div className="alert alert-danger">{t.failed}</div>}
      ...
      {cluster && cluster.available !== false && <div className="provisioning-source-card"><div className="provisioning-mode-content">
        ...
        {form.templateProfileId && <>
          <div className="slider-grid">
            ...
            <label className="form-group"><span>{t.disk} · {form.diskGb} GB</span><input className="resource-range" type="range" min="4" max={Math.min(cluster.maxDiskGb || 64, 64)} step="1" value={form.diskGb} style={rangeStyle(form.diskGb, 4, Math.min(cluster.maxDiskGb || 64, 64))} onChange={e => setField('diskGb', snapWeighted(e.target.value, { min: 4, max: Math.min(cluster.maxDiskGb || 64, 64), minorStep: 2, majorStep: 8 }))} /></label>
          </div>

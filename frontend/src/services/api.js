...
const MESSAGE_TRANSLATIONS = {
  ...
  "Upload a portal package before starting the portal update": "Lade zuerst ein Portal-Paket hoch.",
  "No uploaded portal package is ready": "Es ist kein hochgeladenes Portal-Paket bereit.",
  "Portal package is empty": "Das Portal-Paket ist leer.",
  "Portal package is too large": "Das Portal-Paket ist zu groß.",
  "Portal package must be a ZIP archive": "Das Portal-Paket muss eine ZIP-Datei sein.",
  "The ZIP does not look like a Hosting Portal package": "Die ZIP-Datei scheint kein Hosting-Portal-Paket zu sein.",
  "The host updater helper must be refreshed for local package updates. Run ./setup-updater.sh as root again in /opt/hosting.techbygiusi.com.": "Der Host-Updater muss einmal für lokale Paket-Updates aktualisiert werden. Führe ./setup-updater.sh erneut als root in /opt/hosting.techbygiusi.com aus.",
  "Choose a portal version to roll back to": "Wähle eine Portal-Version für das Rollback aus.",
  "The selected rollback version was not found": "Die ausgewählte Rollback-Version wurde nicht gefunden.",
  "The selected portal version is already installed": "Die ausgewählte Portal-Version ist bereits installiert.",
  "The selected rollback package is no longer available on disk": "Das ausgewählte Rollback-Paket ist nicht mehr auf der Festplatte vorhanden.",
  "The selected portal package is no longer available": "Das ausgewählte Portal-Paket ist nicht mehr verfügbar.",
  ...
  'Management page URL is required': 'Bitte gib eine URL für die Verwaltungsseite ein.',
  'Management page URL must be a valid http:// or https:// URL': 'Die Verwaltungsseite muss eine gültige URL mit http:// oder https:// sein.',
  'Management page URL must not contain login credentials': 'Die URL der Verwaltungsseite darf keine Anmeldedaten enthalten.',
  'Management page saved': 'Die Verwaltungsseite wurde gespeichert.',
  'Management page removed': 'Die Verwaltungsseite wurde entfernt.',
  ...
  'Public TCP ports must be between 20000 and 26000': 'Öffentliche TCP-Ports müssen zwischen 20000 und 26000 liegen.',
  'Public UDP ports must be between 20000 and 26000': 'Öffentliche UDP-Ports müssen zwischen 20000 und 26000 liegen.',
  ...
  'Admin-provided credentials cannot be deleted': 'Vom Admin hinterlegte Zugangsdaten können nicht gelöscht werden.',
  'Administrator-provided service URLs are read-only': 'Vom Administrator hinterlegte Service-URLs sind schreibgeschützt.',
  'Administrator-provided management access is read-only': 'Vom Administrator hinterlegter Verwaltungszugriff ist schreibgeschützt.',
  'Administrator-provided access settings are read-only': 'Vom Administrator hinterlegte Zugriffseinstellungen sind schreibgeschützt.',
  'Administrator-provided service IP and SSH settings are read-only': 'Vom Administrator hinterlegte Service-IP- und SSH-Einstellungen sind schreibgeschützt.',
  'Website link must start with http:// or https://': 'Die Webseiten-URL muss mit http:// oder https:// beginnen.',
...
export const adminApi = {
  ...
  updateInfrastructureNotificationPreferences: (data) => apiClient.put('/admin/settings/infrastructure-notifications', data),
  getPangolinSettings: (clusterId) => apiClient.get(`/admin/pangolin-settings${clusterId ? `?clusterId=${encodeURIComponent(clusterId)}` : ''}`),
  getPangolinPublications: (clusterId) => apiClient.get(`/admin/pangolin-publications${clusterId ? `?clusterId=${encodeURIComponent(clusterId)}` : ''}`),
  deletePangolinPublication: (publicationId) => apiClient.delete(`/admin/pangolin-publications/${publicationId}`),
  updatePangolinSettings: (data, clusterId) => apiClient.put('/admin/pangolin-settings', { ...data, clusterId: clusterId || data?.clusterId }),
  ...
  uploadSystemUpdatePackage: (file, onUploadProgress) => apiClient.post('/admin/system-update/package', file, {
    headers: {
      'Content-Type': 'application/zip',
      'X-Package-Filename': encodeURIComponent(file?.name || 'hosting-portal-package.zip')
    },
    onUploadProgress
  }),
  startSystemUpdate: (type, data = {}) => apiClient.post(`/admin/system-update/${encodeURIComponent(type)}`, data),
  rollbackSystemUpdatePackage: (packageId) => apiClient.post('/admin/system-update/rollback', { packageId }),
...
export const userApi = {
  ...
  getManagementPage: (resourceId) => apiClient.get(`/user/resources/${resourceId}/management-page`),
  saveManagementPage: (resourceId, data) => apiClient.put(`/user/resources/${resourceId}/management-page`, data),
  removeManagementPage: (resourceId) => apiClient.delete(`/user/resources/${resourceId}/management-page`),

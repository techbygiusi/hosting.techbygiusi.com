...
- Introduced a new "Bento" visual design across the entire portal: a pastel, colour-coded compartment system (mint/sky/lavender/peach/blush/butter) replacing the previous sage-green editorial look, with no change to any existing feature or workflow.
- Replaced the design tokens (colours, radius, shadows, spacing) and the shared button, form, alert, card, modal, tab and status-badge styles in `globals.css` with the new pastel palette and rounded, soft-shadow "bento box" treatment.
- Added two new typefaces: Fredoka for headings/display text and Plus Jakarta Sans for body, form and table text.
- Restyled the login, password-reset and setup pages, including a new decorative dot-grid brand panel and colour-coded feature icons.
- Gave dashboard stat tiles and cluster overview cards a bento-style colour rotation, and resource cards a status-coloured accent border (mint = running, blush = stopped).
- Restyled the in-browser terminal chrome and softened the Nord Frost terminal palette to match the new colours, without changing terminal behaviour.
- Rebuilt all outgoing e-mail templates (password reset, welcome, maintenance, monitoring and infrastructure alerts, test mail) with the new pastel theme in `emailTemplates.js`.
